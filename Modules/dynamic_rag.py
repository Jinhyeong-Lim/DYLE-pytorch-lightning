...
sys.path.append("/tmp/pycharm_project/DYLE")
...
class DynamicRagModel(DynamicRagPreTrainedModel):
    ...
    def forward(
        ...
        ## 수정 부분
        # r_probs = torch.softmax(doc_scores, dim=1).clamp(min=1e-5)
        # assert 1==0, print(context_input_ids.shape, decoder_input_ids.shape)
...
class DynamicRagForGeneration(DynamicRagPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            use_cache = False
        # assert 1==0, print(context_input_ids.shape, n_docs)
        outputs = self.rag(
            input_ids=input_ids,
